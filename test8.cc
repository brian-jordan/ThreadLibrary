...
int main(){
	...
	else if (thread_unlock(lock) == -1){
		cout << "thread_unlock failed\n";
	}

	cout << "Project\n";

	else if (thread_signal(lock,cv) == -1){
		cout << "thread_signal failed\n";
	}

	cout << "Makes\n";

	else if (thread_broadcast(lock,cv) == -1){
		cout << "thread_broadcast failed\n";
	}

	cout << "Me\n";

	else if (thread_wait(lock,cv) == -1){
		cout << "thread_wait failed\n";
	}

	cout << "Sad\n";

	else if (thread_yield() == -1){
	...
	else if (thread_create(((thread_startfunc_t) thread1), (void*) NULL) == -1){
